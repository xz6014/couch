...
public class TimeSeries {
	...
	public abstract class Series {
		public enum ID {Root, Style, Goal, Contact, Phase, Hand, Alignment, Length};
	...
	public class Contact : Series {
		...
		public Matrix4x4[][] Transformations;
		public Matrix4x4[][] CorrectedTransformations;
		
		public Matrix4x4[][] InitialCorrectedTransformations;
		
		public float[][] LocalPhases;


		public float[][] ValuesPred;
		public Matrix4x4[][] TransformationsPred;
		public Matrix4x4[][] CorrectedTransformationsPred;
			
		// For Sebastian's Phases
		public float[][] Amplitudes;
		public float[][] Frequencies;
		public Vector2[][] LocalPhaseVectors;
		...
		public Contact(TimeSeries timeSeries, params string[] bones) {
			...
			Bones = bones;
			Values = new float[TimeSeries.Samples.Length][];
			ValuesPred = new float[TimeSeries.Samples.Length][];
			
			Speed = new float[TimeSeries.Samples.Length][];
			LocalPhases = new float[TimeSeries.Samples.Length][];



			for(int i=0; i<Values.Length; i++) {
				Values[i] = new float[Bones.Length];
				ValuesPred[i] = new float[Bones.Length];
				Speed[i] = new float[4];
				LocalPhases[i] = new float[4];
			}

			Transformations = new Matrix4x4[TimeSeries.Samples.Length][];
			CorrectedTransformations = new Matrix4x4[TimeSeries.Samples.Length][];
			InitialCorrectedTransformations = new Matrix4x4[TimeSeries.Samples.Length][];

			TransformationsPred = new Matrix4x4[TimeSeries.Samples.Length][];
			CorrectedTransformationsPred = new Matrix4x4[TimeSeries.Samples.Length][];



			// Speeds = new float[TimeSeries.Samples.Length];
			for(int i=0; i<Transformations.Length; i++) {
					Transformations[i] = new Matrix4x4[4];
					CorrectedTransformations[i] = new Matrix4x4[4];
					InitialCorrectedTransformations[i] = new Matrix4x4[4];

					TransformationsPred[i] = new Matrix4x4[4];
					CorrectedTransformationsPred[i] = new Matrix4x4[4];

			}
			for(int i=0; i<Transformations.Length; i++) {
				for(int j=0; j<4; j++) {
					Transformations[i][j] = Matrix4x4.identity;
					CorrectedTransformations[i][j] = Matrix4x4.identity;
					InitialCorrectedTransformations[i][j] = Matrix4x4.identity;

					TransformationsPred[i][j] = Matrix4x4.identity;
					CorrectedTransformationsPred[i][j] = Matrix4x4.identity;
		...
		public Matrix4x4 GetContactTransformation(int index, int boneindex) {

			return Transformations[index][boneindex];
		}

		public Matrix4x4 GetCorrectedTransformation(int index, int boneindex) {

			return CorrectedTransformations[index][boneindex];
		}

		public void SetCorrectedPosition(int index, Vector3 position, int boneindex) {
			Matrix4x4Extensions.SetPosition(ref CorrectedTransformations[index][boneindex], position);
		}

		public void SetCorrectedRotation(int index, Quaternion Rotation, int boneindex) {
			Matrix4x4Extensions.SetRotation(ref CorrectedTransformations[index][boneindex], Rotation);
		}


		public Vector3 GetTargetHandPosition(int index, int boneindex) {
			return InitialCorrectedTransformations[index][boneindex].GetPosition();
		}

		public Quaternion GetTargetHandRotation(int index, int boneindex) {
			return InitialCorrectedTransformations[index][boneindex].GetRotation();
		}


		public float[] GetContactsPred(int index) {
			return GetContactsPred(index, Bones);
		}

		public float[] GetContactsPred(int index, params string[] bones) {
			float[] values = new float[bones.Length];
			for(int i=0; i<bones.Length; i++) {
				values[i] = GetContactPred(index, bones[i]);
			}
			return values;
		}

		public float GetContactPred(int index, string bone) {
			int idx = ArrayExtensions.FindIndex(ref Bones, bone);
			if(idx == -1) {
				Debug.Log("Contact " + bone + " could not be found.");
				return 0f;
			}
			return ValuesPred[index][idx];
		}

		public Matrix4x4 GetContactTransformationPred(int index, int boneindex) {

			return TransformationsPred[index][boneindex];
		}

		public Matrix4x4 GetCorrectedTransformationPred(int index, int boneindex) {

			return CorrectedTransformationsPred[index][boneindex];
		}

		public void SetCorrectedPositionPred(int index, Vector3 position, int boneindex) {
			Matrix4x4Extensions.SetPosition(ref CorrectedTransformationsPred[index][boneindex], position);
		}
	...
	public class Phase : Series {
		...
		public Phase(TimeSeries timeSeries) {
			...
			LocalPhaseVectors = new Vector2[TimeSeries.Samples.Length][];
			Amplitudes = new float[TimeSeries.Samples.Length][];
			Frequencies = new float[TimeSeries.Samples.Length][];

			for (int i=0; i<TimeSeries.Samples.Length; i++){
				Amplitudes[i] = new float[NPhaseChannels];
				Frequencies[i] = new float[NPhaseChannels];
			}

			for(int i=0; i<TimeSeries.Samples.Length; i++) {
				LocalPhaseVectors[i] = new Vector2[NPhaseChannels];

				for(int j=0; j<NPhaseChannels; j++) {
					LocalPhaseVectors[i][j] = Vector2.zero;
				}
			}
		}
	...
	public class Hand : Series {
		public Matrix4x4[][] Transformations;		
		public Matrix4x4[][] InitialTransformations;
		public Vector3[][] Velocities;

		public override ID GetID() {
			return Series.ID.Hand;
		}

		public Hand(TimeSeries timeSeries) {
			timeSeries.Add(this);
			Transformations = new Matrix4x4[TimeSeries.Samples.Length][];
			Velocities = new Vector3[TimeSeries.Samples.Length][];
			InitialTransformations = new Matrix4x4[TimeSeries.Samples.Length][];
			// Speeds = new float[TimeSeries.Samples.Length];
			for(int i=0; i<Transformations.Length; i++) {
					Transformations[i] = new Matrix4x4[2];
					Velocities[i] = new Vector3[2];
					InitialTransformations[i] = new Matrix4x4[2];
			}
			for(int j=0; j<2; j++) {
				for(int i=0; i<Transformations.Length; i++) {
					Transformations[i][j] = Matrix4x4.identity;
					Velocities[i][j] = Vector3.zero;
					InitialTransformations[i][j] =Matrix4x4.identity;
					// Speeds[i] = 0f;
				}
			}
		}

		public void SetTransformation(int index, Matrix4x4 transformation, int boneindex) {
			Transformations[index][boneindex] = transformation;
		}

		public Matrix4x4 GetTransformation(int index, int boneindex) {
			return Transformations[index][boneindex];
		}

		public void SetPosition(int index, Vector3 position, int boneindex) {
			// for(int i=0; i<position.Length; i++){
			Matrix4x4Extensions.SetPosition(ref Transformations[index][boneindex], position);
			// }
		}

		public Vector3 GetPosition(int index, int boneindex) {
			return Transformations[index][boneindex].GetPosition();
		}

		public void SetRotation(int index, Quaternion rotation, int boneindex) {
			// for(int i=0; i<rotation.Length; i++){
			Matrix4x4Extensions.SetRotation(ref Transformations[index][boneindex], rotation);
			// }
		}

		public Quaternion GetRotation(int index, int boneindex) {
			return Transformations[index][boneindex].GetRotation();
		}
		...
		public void SetVelocity(int index, Vector3 velocity, int boneindex) {
			Velocities[index][boneindex] = velocity;
		}

		public Vector3 GetVelocity(int index, int boneindex) {
			return Velocities[index][boneindex];
		}

		public void SetInitialPosition(int index, Vector3 position, int boneindex) {
			Matrix4x4Extensions.SetPosition(ref InitialTransformations[index][boneindex], position);	
		}

		public Vector3 GetTargetHandPosition(int index, int boneindex) {
			return InitialTransformations[index][boneindex].GetPosition();
		}

		public void SetInitialRotation(int index, Quaternion rotation, int boneindex) {
			// for(int i=0; i<rotation.Length; i++){
			Matrix4x4Extensions.SetRotation(ref InitialTransformations[index][boneindex], rotation);
			// }
		}

		public Quaternion GetTargetHandRotation(int index, int boneindex) {
			return InitialTransformations[index][boneindex].GetRotation();
		}

		public void SetInitialTransformations(int index, Matrix4x4 transformation, int boneindex) {
			InitialTransformations[index][boneindex] = transformation;
		}

		public Matrix4x4 GetInitialTransformations(int index, int boneindex) {
			return InitialTransformations[index][boneindex];
		}
		// public float ComputeSpeed() {
		// 	float length = 0f;
		// 	for(int i=TimeSeries.Pivot+TimeSeries.Resolution; i<TimeSeries.Samples.Length; i+=TimeSeries.Resolution) {
		...
		public void Draw() {
			...
			//Connections
			for (int j=0; j<2; j+=1) {
				for(int i=0; i<Transformations[0].Length-step; i+=step) {
					UltiDraw.DrawLine(Transformations[i][j].GetPosition(), Transformations[i+step][j].GetPosition(), 0.01f, UltiDraw.Black);
				}
			}

			//Positions
			for (int j=0; j<2; j+=1) {
				for(int i=0; i<Transformations[0].Length; i+=step) {
					UltiDraw.DrawCircle(Transformations[i][j].GetPosition(), 0.025f, UltiDraw.Black);
				}
			}
			...
			for (int j=0; j<2; j+=1) {
				for(int i=0; i<Velocities.Length; i+=step) {
					UltiDraw.DrawLine(Transformations[i][j].GetPosition(), Transformations[i][j].GetPosition() + Velocities[i][j], 0.025f, 0f, UltiDraw.DarkGreen.Transparent(0.25f));
				}
			}
