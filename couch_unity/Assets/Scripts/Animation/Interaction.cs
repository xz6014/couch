...
public class Interaction : MonoBehaviour {
    ...
    public void AddContact(string name, Vector3 position, Quaternion rotation)
    {
        AddContact();
        Contacts.Last().gameObject.name = name;
        Contacts.Last().gameObject.transform.position = position;
        Contacts.Last().gameObject.transform.rotation = rotation;
    }

    public void AddContact(Vector3 position, Quaternion rotation)
    {
        AddContact();
        Contacts.Last().gameObject.transform.position = position;
        Contacts.Last().gameObject.transform.rotation = rotation;
	...
	void OnRenderObject() {
		...
		if (ShowContacts)
        {
            ...
            for (int i = 0; i < Contacts.Length; i++)
            {
                ...
                {
                    UltiDraw.DrawSphere(Contacts[i].position, Contacts[i].rotation, 0.05f, colors[i]);
                    // UltiDraw.DrawWiredSphere(Contacts[i].position, Contacts[i].rotation, 0.1f, colors[i], UltiDraw.Black);
                    // UltiDraw.DrawForwardGizmo(Contacts[i].position, Contacts[i].rotation, 0.4f);
