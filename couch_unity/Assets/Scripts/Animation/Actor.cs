...
public class Actor : MonoBehaviour {
	...
	public void Draw(Color boneColor, Color jointColor, float alpha) {
		...
		if(DrawAlignment) {
			foreach(Bone bone in Bones) {
				UltiDraw.DrawLine(bone.Transform.position, bone.Transform.position + bone.Transform.rotation * bone.GetAlignment(), 0.05f, 0f, UltiDraw.Magenta);
			}
		}
		// UltiDraw.End();
		...
		if(DrawHistory) {
			if(DrawSkeleton) {
				// Color[] colors = UltiDraw.GetRainbowColor(History.Count /4 ) ;

				for(int i=0; i<History.Count; i++) {
					if (i % 6 ==0 ){
						Sketch(History[i].Transformations, UltiDraw.GetRainbowColor(4, 5).Transparent(0.5f));
					}
				}
			}
			if(DrawVelocities) {
	...
	public class Bone {
		...
		public void ComputeAlignment() {
			if(Childs.Length != 1) {
				...
				Alignment = (GetChild(0).Transform.position - Transform.position).GetRelativeDirectionTo(Transform.GetWorldMatrix(true));
			}
		}
		...
		public void RestoreAlignment() {
			...
			Vector3 childPosition = GetChild(0).Transform.position;
			Quaternion childRotation = GetChild(0).Transform.rotation;
			Vector3 target = (childPosition-position);
			Vector3 aligned = rotation * Alignment;
			Transform.rotation = Quaternion.FromToRotation(aligned, target) * rotation;
			GetChild(0).Transform.position = position + Alignment.magnitude * target.normalized;
			GetChild(0).Transform.rotation = childRotation;
		}
	}
	...
	public class Actor_Editor : Editor {
		...
		public override void OnInspectorGUI() {
			...
			Target.DrawAlignment = EditorGUILayout.Toggle("Draw DrawAlignment", Target.DrawAlignment);
