...
public class PhaseModule : Module {
	...
	public float Delta = 1.05f;

	private bool Optimising = false;

	//private MotionReference Reference = null;
	...
	public override Module Initialise(MotionData data) {
		...
		ToggleVariable(Data.Source.FindBone("m_avg_R_Ankle").Index);
		//M: temp to automatically start optimizating the phase
		// Inspect = true;
	...
	public override void Slice(Sequence sequence) {
		RegularPhaseFunction.Phase = ArrayExtensions.Gather(ref RegularPhaseFunction.Phase, sequence.GetIndices());
		RegularPhaseFunction.Keys = ArrayExtensions.Gather(ref RegularPhaseFunction.Keys, sequence.GetIndices());

		InversePhaseFunction.Phase = ArrayExtensions.Gather(ref InversePhaseFunction.Phase, sequence.GetIndices());
		InversePhaseFunction.Keys = ArrayExtensions.Gather(ref InversePhaseFunction.Keys, sequence.GetIndices());
	}
	...
	public void SetMaximumVelocity(float value) {
		...
		if(MaximumVelocity != value) {
			...
			RegularPhaseFunction.Compute();
			InversePhaseFunction.Compute();
		}
	}
	...
	public Vector2 GetLocalPhaseVector(Frame frame, bool mirrored, int channel_index) {
		return mirrored ? InversePhaseFunction.GetLocalPhaseVector(frame, channel_index) : RegularPhaseFunction.GetLocalPhaseVector(frame, channel_index);
	}	
	
	public float GetAmplitude(Frame frame, bool mirrored, int channel_index) {
		return mirrored ? InversePhaseFunction.GetAmplitude(frame, channel_index) : RegularPhaseFunction.GetAmplitude(frame, channel_index);
	}

	public float GetFrequency(Frame frame, bool mirrored, int channel_index) {
		return mirrored ? InversePhaseFunction.GetFrequency(frame, channel_index) : RegularPhaseFunction.GetFrequency(frame, channel_index);
	}

	public void SetLocalPhases(bool mirrored ,float[][] features) {
		if (mirrored){
			InversePhaseFunction.LoadFrequencyLocalPhase(features); 
		}
		else{
			RegularPhaseFunction.LoadFrequencyLocalPhase(features);
		}
	}
	...
	protected override void DerivedDraw(MotionEditor editor) {
		...
		*/
		/*
		if(Reference != null) {
			Reference.Draw(editor);
		}
		*/
	}

	protected override void DerivedInspector(MotionEditor editor) {
		RegularPhaseFunction.Setup();
		InversePhaseFunction.Setup();

		/*
		if(Utility.GUIButton("Swap", UltiDraw.DarkGrey, UltiDraw.White)) {
			Utility.Swap(ref RegularPhaseFunction, ref InversePhaseFunction);
		}
		*/
		...
		Delta = EditorGUILayout.FloatField("Default Delta", Delta);
		int index = EditorGUILayout.Popup("Phase Detector", 0, ArrayExtensions.Concat("Select...", Data.Source.GetBoneNames()));
		if(index > 0) {
		...
		for(int i=0; i<Data.Source.Bones.Length; i++) {
			if(Variables[i]) {
				using(new EditorGUILayout.VerticalScope ("Box")) {
					...
					if (Data.Source.Bones[i].Name.Contains("_L_")) {
						EditorGUILayout.LabelField(Data.Source.Bones[i].Name);
						EditorGUILayout.LabelField(Data.Source.Bones[Data.Symmetry[i]].Name.Replace("_L_", "_R_"));
					}
					else
						if (Data.Source.Bones[i].Name.Contains("_R_")) {
						EditorGUILayout.LabelField(Data.Source.Bones[i].Name);
						EditorGUILayout.LabelField(Data.Source.Bones[Data.Symmetry[i]].Name.Replace("_R_", "_L_"));
						}
		...
		/*
		Reference = (MotionReference)EditorGUILayout.ObjectField("Reference", Reference, typeof(MotionReference), true);
		if(Reference != null) {
			Reference.Inspector(editor);
		}
		*/
	}
	...
	public class PhaseFunction {
		...
		public void LoadFrequencyLocalPhase(float [][] features) {
			NPhaseChannels = 5;

			LocalPhaseVectors = new Vector2[Module.Data.GetTotalFrames()][];
			Amplitudes = new float[Module.Data.GetTotalFrames()][];
			Frequencies = new float[Module.Data.GetTotalFrames()][];
			...
			for (int b=0; b<NPhaseChannels; b++){
				for (int l=0; l<Module.Data.GetTotalFrames(); l++){
					Vector2 tmp = new Vector2(features[l][b*2], features[l][b*2 + 1]);  // because of root phase is not considered
					LocalPhaseVectors[l][b] = tmp.normalized;
					Amplitudes[l][b] = tmp.magnitude;
					Frequencies[l][b] = features[l][NPhaseChannels * 2 + b];
				}
			}
			// Debug.Log(LocalP/haseVectors[0].Length);
			...
			for(int i=0; i<Velocities.Length; i++) {
				for(int j=0; j<Module.Variables.Length; j++) {
					if(Module.Variables[j]) {
						float boneVelocity = Mathf.Min(Module.Data.Frames[i].GetBoneVelocity(j,  (this == Module.RegularPhaseFunction ? false : true), 1f/Module.Data.Framerate).magnitude, Module.MaximumVelocity);
		...
		public Vector2 GetLocalPhaseVector(Frame frame, int channel_index) {
			return LocalPhaseVectors[frame.Index-1][channel_index];
		}

		public float GetAmplitude(Frame frame, int channel_index) {
			return Amplitudes[frame.Index-1][channel_index];
		}		
		
		public float GetFrequency(Frame frame, int channel_index) {
			return Frequencies[frame.Index-1][channel_index];
		}
	...
	public class PhaseEvolution {
		...
		public void Assign() {
			...
			for(int i=0; i<Populations.Length; i++) {
				for(int j=Populations[i].Interval.Start; j<=Populations[i].Interval.End; j++) {
					Function.NCycle[j] = Utility.Normalise(Function.Cycle[j], min, max, 0f, 1f);
				}
			}
