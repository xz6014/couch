...
public class RootModule : Module {
	...
	public void DetectSetup() {
		MotionData.Hierarchy.Bone rs = Data.Source.FindBoneContains("m_avg_R_Shoulder");
		RightShoulder = rs == null ? 0 : rs.Index;
		MotionData.Hierarchy.Bone ls = Data.Source.FindBoneContains("m_avg_L_Shoulder");
		LeftShoulder = ls == null ? 0 : ls.Index;
		MotionData.Hierarchy.Bone rh = Data.Source.FindBoneContains("m_avg_R_Hip");
		RightHip = rh == null ? 0 : rh.Index;
		MotionData.Hierarchy.Bone lh = Data.Source.FindBoneContains("m_avg_L_Hip");
		LeftHip = lh == null ? 0 : lh.Index;
		MotionData.Hierarchy.Bone n = Data.Source.FindBoneContains("m_avg_Neck");
		Neck = n == null ? 0 : n.Index;
		MotionData.Hierarchy.Bone h = Data.Source.FindBoneContains("m_avg_Pelvis");
		Hips = h == null ? 0 : h.Index;
		Ground = LayerMask.GetMask("Ground");
	}

	public Matrix4x4 GetRootTransformation(Frame frame, bool mirrored) {
		// Debug.Log(GetRootRotation(frame, mirrored));
	...
	public Vector3 GetRootPosition(Frame frame, bool mirrored) {
		// Debug.Log(frame.GetBoneTransformation(0, mirrored).GetPosition());
