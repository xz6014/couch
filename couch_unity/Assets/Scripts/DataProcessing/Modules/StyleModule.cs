...
public class StyleModule : Module {
	...
	/*
	public float[,] GetSignalHistory(Frame frame, int history) {
	...
	public float[] GetInverseSignal(Frame frame) {
		float[] signal = new float[Functions.Length];
		for(int i=0; i<signal.Length; i++) {
	...
	protected override void DerivedInspector(MotionEditor editor) {
		...
		*/
		...
		for(int j=start; j<=end; j++) {
			Frame f = Data.GetFrame(j);
			float[] signal = GetSignal(Data.GetFrame(j));
			for(int i=0; i<Functions.Length; i++) {
				if(signal[i] > 0f) {
					float _start = (float)(Mathf.Clamp(f.Index, start, end)-start) / (float)elements;
					float _end = _start;
					float xStart = rect.x + _start * rect.width;
					float xEnd = rect.x + _end * rect.width;
					float yStart = rect.y + 0.5f*rect.height - 0.5f*signal[i]*rect.height;
					float yEnd = rect.y + 0.5f*rect.height + 0.5f*signal[i]*rect.height;
					UltiDraw.DrawLine(new Vector3(xStart, yStart, 0f), new Vector3(xEnd, yEnd, 0f), colors[i].Transparent(0.5f));
				}
			}
		}
		*/
