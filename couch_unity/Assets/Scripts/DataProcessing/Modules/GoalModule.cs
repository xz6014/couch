...
public class GoalModule : Module {
	...
	// public float GetAction(Frame frame, float delta, string action) {
	// 	//Delta for taking the goal from one frame ahead when being at the current frame.
	// 	float action;;
	// 	for(int i=0; i<Functions.Length; i++) {
	// 		if (Function[i].Name == action){
	// 			action = Functions[i].GetValue(Data.GetFrame(frame.Timestamp - 1f/Data.Framerate + delta));
	// 		}
	// 	}
	// 	return action;
	// }
	...
	public class TargetFunction {
		...
		public TargetFunction(GoalModule module) {
			...
			for(int i=0; i<Specifiers.Length; i++) {
				Specifiers[i] = "m_avg_Pelvis";
			}
		}



		public Matrix4x4 GetGoalTransformation(Frame reference, float offset, bool mirrored, float delta, bool body_goal) {
			return Matrix4x4.TRS(GetGoalPosition(reference, offset, mirrored, delta, body_goal), GetGoalRotation(reference, offset, mirrored, delta, body_goal), Vector3.one);
		}

		public Vector3 GetGoalPosition(Frame reference, float offset, bool mirrored, float delta, bool body_goal) {
			use_body = UseBodyGoal();
			...
			if((GetPivot(frame) == Pivot.DynamicObject || GetPivot(frame) == Pivot.StaticObject)) {
				...
				if(interaction == null) {
					...
					if (use_body){
						return Module.Data.GetFrame(reference.Index).GetBoneTransformation("m_avg_Pelvis", mirrored).GetPosition();
					}
					else{
		...
		public Quaternion GetGoalRotation(Frame reference, float offset, bool mirrored, float delta, bool body_goal) {
			use_body = UseBodyGoal();
			...
			if((GetPivot(frame) == Pivot.DynamicObject || GetPivot(frame) == Pivot.StaticObject)) {
				...
				if(interaction == null) {
					...
					if (use_body){
						return Quaternion.LookRotation(Vector3.ProjectOnPlane(Module.Data.GetFrame(reference.Index).GetBoneTransformation("m_avg_Pelvis", mirrored).GetForward(), Vector3.up).normalized, Vector3.up);
					}
					else{
						if(interaction.ContainsContact(GetSpecifier(frame))) {
