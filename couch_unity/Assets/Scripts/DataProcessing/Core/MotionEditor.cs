...
public class MotionEditor : MonoBehaviour {
	...
	public Frame GetPreviousFrame() {
		return Data == null ? null : Data.GetFrame(Timestamp - 1/TargetFramerate);
	}
	...
	public void LoadAllData() {
		...
		// 	return;
		// }
		foreach(MotionData data in Files){
			LoadData(data);
		}
	}
	...
	public void LoadFrame(float timestamp) {
		...
		if (frame.Index % 3 == 0){
		this.SetRandomSeed(frame.Index);
		}
	...
	public void PlayAnimationAll() {
		...
		foreach (MotionData data in Files){
			...
			EditorCoroutines.StartCoroutine(PlayAll(data), this);
		}
	}
	...
	public void StopAnimationAll() {
		...
		Playing = false;
		foreach (MotionData data in Files){
			EditorCoroutines.StopCoroutine(PlayAll(data), this);
		}
	}


	private IEnumerator PlayAll(MotionData data) {
	...
	public class MotionEditor_Editor : Editor {
		...
		void Awake() {
			...
			// Target.Data.AddSpecificSequence(2, Target.Data.GetTotalFrames() - 30);
		...
		public void Inspector() {
			...
			// Target.Data.AddSpecificSequence(2, Target.Data.GetTotalFrames() - 30);
			// Target.Data.AddSpecificSequence(1, Target.Data. - 30);
					...
					if(Target.Data != null) {
						...
						using(new EditorGUILayout.VerticalScope ("Box")) {
							...
							using(new EditorGUILayout.VerticalScope ("Box")) {
								...
								// Debug.Log(EditorGUILayout.IntField(Target.RandomSeed, GUILayout.Width(40f)));
								GUILayout.FlexibleSpace();
								EditorGUILayout.EndHorizontal();
							...
							using(new EditorGUILayout.VerticalScope ("Box")) {
								...
								if(Utility.GUIButton("<", UltiDraw.Grey, UltiDraw.White, 20f, 20f)) {
									// Debug.Log(Target.GetCurrentSeed());
								...
								if(index != frame.Index) {
									...
									// Debug.Log(string.Format("{0} {1}",index, Target.GetCurrentSeed()));
							...
							// EditorGUILayout.BeginHorizontal();
							// if(Utility.GUIButton("Careful!!!!! Load All", Target.Visualise ? UltiDraw.Cyan : UltiDraw.LightGrey, UltiDraw.Black)) {
							// 	Target.LoadAllData();
							// }
							// EditorGUILayout.EndHorizontal();

							// EditorGUILayout.BeginHorizontal();
							// if(Utility.GUIButton("Play All", Target.Visualise ? UltiDraw.Cyan : UltiDraw.LightGrey, UltiDraw.Black)) {
							// 	Target.PlayAnimationAll();
							// }
							// EditorGUILayout.EndHorizontal();

							// EditorGUILayout.BeginHorizontal();
							// if(Utility.GUIButton("Stop All", Target.Visualise ? UltiDraw.Cyan : UltiDraw.LightGrey, UltiDraw.Black)) {
							// 	Target.StopAnimationAll();
							// }
							// EditorGUILayout.EndHorizontal();
							...
							if (Target.Data.Modules.Length < 7){
								Target.Data.AddModule(((Module.ID)(1-1)));
								Target.Data.AddModule(((Module.ID)(2-1)));
								Target.Data.AddModule(((Module.ID)(3-1)));
								Target.Data.AddModule(((Module.ID)(4-1)));
								Target.Data.AddModule(((Module.ID)(5-1)));
								Target.Data.AddModule(((Module.ID)(6-1)));
								Target.Data.AddModule(((Module.ID)(10-1)));
							}
							...
							using(new EditorGUILayout.VerticalScope ("Box")) {
								...
								if(Target.Settings) {
									...
									int [] SymmetricBoneNames = new int[Target.Data.Source.GetBoneNames().Length];
									for(int i=0; i<Target.Data.Source.Bones.Length; i++)  {
										if (Target.Data.Source.GetBoneNames()[i].Contains("_L_")){
											// myList.FindIndex(a => a.Prop == oProp)
											SymmetricBoneNames[i] = System.Array.IndexOf(Target.Data.Source.GetBoneNames(), Target.Data.Source.GetBoneNames()[i].Replace("_L_", "_R_"));
										}
										else if (Target.Data.Source.GetBoneNames()[i].Contains("_R_")){
											SymmetricBoneNames[i] = System.Array.IndexOf(Target.Data.Source.GetBoneNames(), Target.Data.Source.GetBoneNames()[i].Replace("_R_", "_L_"));
										}	
										else {
											SymmetricBoneNames[i] = i;
										}
									}
									for(int j=0; j<Target.Data.Source.Bones.Length; j++) {
										EditorGUILayout.BeginHorizontal();
										EditorGUI.BeginDisabledGroup(true);
										EditorGUILayout.TextField(Target.Data.Source.GetBoneNames()[j]);
										EditorGUI.EndDisabledGroup();
										Target.Data.SetSymmetry(j, EditorGUILayout.Popup(SymmetricBoneNames[j], Target.Data.Source.GetBoneNames()));
										EditorGUILayout.LabelField("Mass", GUILayout.Width(40f));
										Target.Data.Source.Bones[j].Mass = EditorGUILayout.FloatField(Target.Data.Source.Bones[j].Mass);
										EditorGUILayout.LabelField("Alignment", GUILayout.Width(60f));
										Target.Data.Source.Bones[j].Alignment = EditorGUILayout.Vector3Field("", Target.Data.Source.Bones[j].Alignment);
										EditorGUILayout.EndHorizontal();
									}
								}
