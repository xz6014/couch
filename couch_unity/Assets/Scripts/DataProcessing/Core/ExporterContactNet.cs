...
public class ExporterContactNet : EditorWindow {
	...
	[MenuItem ("AI4Animation/Exporter ContactNet")]
	static void Init() {
		Window = EditorWindow.GetWindow(typeof(ExporterContactNet));
		Scroll = Vector3.zero;
	}
	...
	private IEnumerator ExportData() {
		if(Editor == null) {
			...
			StreamWriter SequenceNames = CreateFile("SequencesNames");
			...
			for(int i=0; i<Files.Count; i++) {
				...
				if(Export[i]) {
					...
					// hands_additional_key_frame = hands_modes_int[i];
					
					///////////////// TODO ONLY Actual Contacts //////////////////////////////////////////////
					...
					StyleModule StyleModule = (StyleModule)Editor.GetData().GetModule(Module.ID.Style);
							...
							Debug.Log("File: " + Editor.GetData().GetName() + " Scene: " + Editor.GetData().GetName() + " " + (Editor.Mirror ? "[Mirror]" : "[Default]"));
								...
								foreach(Sequence seq in Editor.GetData().Sequences) {
									...
									while(start+(sample+1)/Framerate <= end-2/Framerate) {
										...
										if (StyleModule.GetStyle(Files[i].GetFrame(start+sample/Framerate), "Sit") > 0.5f){

											InputData current = new InputData(Editor, start+sample/Framerate);
											OutputData next = new OutputData(Editor, start+(sample + 1)/Framerate);
											Editor.SetRandomSeed(current.Frame.Index);



											GameObject[] gameObjects = Utility.Unroll(Editor.GetData().GetScene().GetRootGameObjects());

											GameObject chair = gameObjects[0];
											Matrix4x4 object_root = current.Root;

											foreach (GameObject gameObject in gameObjects){
												if(gameObject.GetComponent<Interaction>() != null & !gameObject.name.Contains("mirror") & gameObject.active){
													// Debug.Log(string.Format("{0} {1}", sample, gameObject.name));
													chair = gameObject;
													object_root = chair.GetComponent<Interaction>().GetCenter();
											...
											for(int k=0; k<BoneNames.Count; k++){
												// if (current.ContactSeries.TransformationsPred[current.TimeSeries.Pivot][k].GetPosition() != Vector3.zero){
												sum += current.ContactSeries.TransformationsPred[current.TimeSeries.Pivot][k].GetPosition().magnitude;
											...
											if (sum > 0f){
												...
												if (m == 1){
													SequenceNames.WriteLine(Files[i].name.ToString());
												}
												else{
													SequenceNames.WriteLine(Files[i].name.ToString() + "_mirrored");
												}
												...
												// Matrix4x4 object_root = chair.GetComponent<Interaction>().GetCenter();

												//INPUT
												//Auto-Regressive Posture
												for(int k=0; k<current.Posture.Length; k++) {
													// X.Feed(current.Posture[k].GetPosition(), "Bone"+(k+1)+Editor.GetActor().Bones[k].GetName()+"Position");
													X.Feed(current.Posture[k].GetPosition().GetRelativePositionTo(object_root), "Bone"+(k+1)+Editor.GetActor().Bones[k].GetName()+"Position");
												...
												// Debug.Log(current.Interaction.Points.Length);
												// Interaction Geometry
												for(int k=0; k<current.Interaction.Points.Length; k++) {
													X.Feed(current.Interaction.References[k].GetRelativePositionTo(object_root), "InteractionPosition"+(k+1));
													X.Feed(current.Interaction.Occupancies[k], "InteractionOccupancy"+(k+1));
												}
												X.Feed(object_root.GetPosition(), "ObjectRoot");
												...
												// OUTPUT
												for(int k=0; k<BoneNames.Count; k++){
													Y.Feed(current.ContactSeries.TransformationsPred[current.TimeSeries.Pivot][k].GetPosition().GetRelativePositionTo(object_root), "HandTrajectory"+"-"+BoneNames[k]);
												}
	...
	private float[] GenerateGatingInteraction(OutputData current) {
	...
	public class InputData {
		...
		public InputData(MotionEditor editor, float timestamp) {
			...
			SensorOffsets = ((ContactModule)editor.GetData().GetModule(Module.ID.Contact)).GetSensorOffsets("m_avg_R_Wrist", "m_avg_L_Wrist", "m_avg_R_Ankle", "m_avg_L_Ankle");
