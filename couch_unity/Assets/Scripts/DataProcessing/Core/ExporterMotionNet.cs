...
public class ExporterMotionNet : EditorWindow {
	...
	[MenuItem ("AI4Animation/Exporter MotionNet")]
	static void Init() {
		Window = EditorWindow.GetWindow(typeof(ExporterMotionNet));
		Scroll = Vector3.zero;
	}
						...
						if(!Editor.Mirror || WriteMirror && Editor.Mirror && Editor.GetData().Symmetric) {
							...
							// Sequence seq = Editor.GetData().GetUnrolledSequence(); {
								...
								while(start+(sample+1)/Framerate <= end-2/Framerate) {
									...
									InputData current = new InputData(Editor, start+sample/Framerate, 0);
									sample += 1;
									OutputData next = new OutputData(Editor, start+sample/Framerate, 0);
									...
									for(int k=0; k<2; k++){
										...
										for(int j=0; j<current.TimeSeries.Samples.Length; j++) {

											Quaternion rot =  current.ContactSeries.GetCorrectedTransformation(j, k).GetRotation();
											Vector3 SensorPosition = current.ContactSeries.GetCorrectedTransformation(j, k).GetPosition() + rot * offset;
											if (current.ContactSeries.Transformations[j][k].GetPosition() == Vector3.zero){
												X.Feed(Vector3.zero, "HandTrajectory"+"-"+BoneNames[k]+(j+1));
											}
											else{
												// Debug.Log(string.Format("{0} {1}", current.Frame.Index, k));
												X.Feed(SensorPosition.GetRelativePositionTo(current.ContactSeries.Transformations[j][k]), "HandTrajectory"+"-"+BoneNames[k]+(j+1));
											}
									...
									for(int k=0; k<next.TimeSeries.Samples.Length; k++) {
										// Debug.Log(current.ContactSeries.GetContacts(k, "m_avg_R_Wrist", "m_avg_L_Wrist")[0]);
										for(int j=0; j<2; j++) {
											X.Feed(current.ContactSeries.LocalPhases[k][j], "ContactSeries"+"-"+BoneNames[j]+(k+1));
										}
									}

									//Gating Variables
									X.Feed(GenerateGatingInteraction(current), "Gating-");
									...
									//Key Contacts
									Y.Feed(next.ContactSeries.GetContacts(next.TimeSeries.Pivot, "m_avg_Pelvis", "m_avg_R_Wrist", "m_avg_L_Wrist", "m_avg_R_Ankle", "m_avg_L_Ankle"), "Contact-");
	...
	public class InputData {
		...
		public InputData(MotionEditor editor, float timestamp, int additional_key_frame) {
	...
	public class OutputData {
		...
		public OutputData(MotionEditor editor, float timestamp, int additional_key_frame) {
			editor.LoadFrame(timestamp);
			Frame = editor.GetCurrentFrame();
